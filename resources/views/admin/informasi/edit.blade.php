<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('informasi.show', $informasi) }}" class="rounded-full border border-emerald-100 bg-white px-4 py-2 text-sm font-semibold text-emerald-700 shadow-sm transition hover:bg-emerald-50">
                Kembali
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="surface-panel p-6 sm:p-8">
                <form method="POST" action="{{ route('informasi.update', $informasi) }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <textarea id="konten" name="konten" rows="8" required
                            class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-900 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">{{ old('konten', $informasi->konten) }}</textarea>
                        @error('konten')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label for="gambar" class="block text-sm font-semibold text-slate-700">Gambar Baru (opsional)</label>
                    ...
                    @if($informasi->gambar)
                        <div class="rounded-2xl border border-slate-200 bg-white p-4">
                            ...
                            <img src="{{ asset('storage/' . $informasi->gambar) }}" alt="{{ $informasi->judul }}" class="mt-3 h-56 w-full rounded-xl object-cover">
                        </div>
                    @endif

                    <div class="flex flex-wrap items-center justify-end gap-3">
                        <a href="{{ route('informasi.show', $informasi) }}" class="rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-600 shadow-sm transition hover:bg-slate-50">
                            Batal
                        </a>
